...
class QuestionsReader extends ExcelReader {
    constructor (filePath) {
        ...
        this.questBeginSheet = this.workbook.Sheets[config.excelSheetNames.questBeginSheet];
    ...
    validate () {
        ...
        // Check les string de textbutton
    ...
    importQuestionsDatas (questSheet, isEnd) {
        ...
        for (let indexQuest = 0; indexQuest < relatedQuestions.length; indexQuest++) {
            ...
            const question = questionsList.find( quest => quest.id === idQuest);
        ...
        // Set the prop of the newConfig question list (Begin or End)
        if (isEnd)
            this.newConfig.questEnd = questionsList;
        else 
            this.newConfig.questBegin = oldConfig.QCM.begin.list.slice(0,2).concat(questionsList);
