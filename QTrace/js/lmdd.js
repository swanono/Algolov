var lmdd = (function () {
    ...
    var options = {//default settings
        ...
        positionDelay: false,
        dataMode: false,
		dragstartlistner: null,
		dragendlistner: null,
		draggablelistner: null
    };
	var click=false;
        ...
        if (undo) {
            tasks[undo].push(function () {
                if (action) {
					...
					if(!logend) return;
					if (options.dragendlistner)
					{
						//if (positions.currentContainer)
						//{
						//	if(positions.currentContainer.id)
						//		options.dragendlistner(mirror.id,positions.currentContainer.id);
						//	else
						//		options.dragendlistner(mirror.id,positions.currentContainer.parentNode.id);
						//}
						//else if(tempContainer)
						//{
						//	if(tempContainer.id)
						//		options.dragendlistner(mirror.id,tempContainer.id);
						//	else
						//		options.dragendlistner(mirror.id,tempContainer.parentNode.id);
						//}
						//else
							options.dragendlistner(mirror.id,null);
						tempContainer=null;
						logend=false;
					}
                }
	...
	function isAcceptChild(container,target)
	{
		if('parentState' in target)
		{ 
			if(target.parentState===container.id)
				return container;
			if('grandParentState' in target && container.parentNode && container.parentNode.parentNode && 
				container.parentNode.parentNode.parentNode && container.parentNode.parentNode.parentNode.parentNode && 
				container.parentNode.parentNode.parentNode.parentNode.id===target.grandParentState)
				return document.getElementById(target.parentState);
		}
		if('childLimit' in container && container.childLimit<=container.childNodes.length && target.parentNode!=container)
			return null;
    ...
    function eventManager(event) {//handle events lifecycle and app status
		if(event.type === "draggablecontainer") 
		{
			let ele=document.getElementById(event.detail.container);
			if(ele)
			{
				positions.currentContainer=ele;
				newContainer(false);
			}
			//if(!mirror)
			//{
			//	let ele=document.getElementById(event.detail.card);
			//	if(ele)
			//	{
        ...
        switch (status) {
            case "waitDragStart":
                if ((event.type === "click") && (event.button === 0)) {//trigger timeout function to enable clicking and text selection
					scope = this;
					logend=true;
					if(!scope)
						scope=event.relatedTarget;
                    lastEvent = event;
					if(!isReplay)
					{
                        ...
                        if (status === "dragStartTimeout") {//no events fired during the timeout
                            if ((scope.lmddOptions.handleClass) && (!event.target.classList.contains(scope.lmddOptions.handleClass))) {//not dragging with handle
                                ...
                                if (!target) {//not dragging a draggable
                                    ...
                                    status = "dragStart";
									if(options.dragstartlistner)
										options.dragstartlistner(target.id,DOMGenerator.GetParentWithId(target.parentElement.id));
            ...
            case "dragStart":
			 if (event.type === "mousemove") {
                ...
                if (click && ((event.type === "click") ||(event.type === "mousedown") || (event.type === "mouseup") || (event.type === "mousemove") && (event.buttons === 0))) {//or mousemove with no buttons in case mouseup event was not fired
                    if (!dragged) {
                        killEvent();
                        return;
                    }
                    ...
                    if (Math.abs(offsetX) + Math.abs(offsetY) > 3) {//make a graceful finish...
                        status = "waitDragEnd";
						if (options.dragendlistner&&tempContainer)
						{
							if (positions.currentContainer)
							{
								if(positions.currentContainer.id)
	...
	function newContainer(recordevent)
	{
		tempContainer=isAcceptChild(positions.currentContainer,dragged);
		if(tempContainer)
		{
			positions.currentContainer=tempContainer;
			if (positions.currentContainer !== positions.previousContainer) {//its a new one...
				updateCurrentCoordinates();
				updateCurrentPosition();
				appendDraggedElement();
				if(recordevent&&options.draggablelistner)
				{
					if(tempContainer.id)
						options.draggablelistner(tempContainer.id,mirror.id);
					else
						options.draggablelistner(DOMGenerator.GetParentWithId(tempContainer).id,mirror.id);
				}
			}
			else {//same container
				updateCurrentPosition();
    ...
    function eventTicker() {//interval function for updating and handling mouse movements while dragging
        /*if (scope&&scope.lmddOption&&!scope.lmddOptions.dataMode.lmddOptions.nativeScroll) {
            scrollManager.updateEvent(lastEvent);
        }
        if (refEvent === lastEvent) {
            return false;
        }
        if(refEvent&&scope&&scope.lmddOption&&scope.lmddOptions.positionDelay){
            if(lastEvent.timeStamp - refEvent.timeStamp < scope.lmddOptions.calcInterval){
                return false;
            }
        }
		if(scope.lmddOptions)
	    {
		...
		/*if (!scope.lmddOptions.nativeScroll || !scope.lmddOptions || !scope.lmddOptions.nativeScroll) 
			return;*/
	 if (!scope.lmddOptions.nativeScroll) {
            scrollManager.updateEvent(lastEvent);
        }
        ...
        if (!positions.currentContainer) {//no container found
            if (positions.previousContainer && scope&&scope.lmddOption&&scope.lmddOptions.revert) {//execute once (revert)
                appendDraggedElement();
            }
        }
    ...
    function killEvent() {//end current drag event
        ...
        if (scope && scope.lmddOptions && scope.lmddOptions.dataMode) {//undo DOM mutations
    ...
    return {//exposed methods
        set: function (el, lmddOptions, dragstartlistner, dragendlistner, draggablelistner) {
            if (!el.lmdd) {
                clean(el);//get rid of whitespaces
                el.lmdd = true;
                el.lmddOptions = assignOptions(options, lmddOptions);//create options object
                el.addEventListener("click", eventManager, false);
				...
				if(dragstartlistner)
					options.dragstartlistner=dragstartlistner;
				if(dragendlistner)
					options.dragendlistner=dragendlistner;	
				if(draggablelistner)
					options.draggablelistner=draggablelistner;	
            }
        },
        unset: function (el) {
            if (el.lmdd) {
                el.removeEventListener("click", eventManager, false);
                el.lmdd = false;
